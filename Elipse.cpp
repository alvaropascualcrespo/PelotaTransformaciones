...
Elipse::Elipse(PV2D* cent, double alt, double anc, bool pint, GLdouble v){
        pintable = pint;
        alto = alt;
        ancho = anc;
        centro = cent;
...
bool Elipse::interseccion(PV2D* P, PV2D* direccion, double &thit, PV2D* &normalIn){
        ...
        GLfloat xaux1 = matrix[0][0] * direccion->x;
        GLfloat yaux1 = matrix[1][1] * direccion->y;
        ...
        normalIn->y = yaux2;
        thit = thit * velocidad;
        //if (thit < velocidad && thit > 0 && choque) ShowMessage("Ahora");
        return choque;
